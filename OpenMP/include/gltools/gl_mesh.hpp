...
struct Vertex{
  ...
  static size_t offsetColor() { return sizeof(glm::vec3) ; }
...
struct Mesh{
  ...
  void buffer(){
     ...
     *-----------------------------------------------------------------------------*/
      ...
      glBufferData( GL_ELEMENT_ARRAY_BUFFER, indices.size() * sizeof(GLushort), &(indices[0]), GL_DYNAMIC_DRAW ); //<-- Update infrequently, if ever
  ...
  void subBuffer(glm::vec4 color){
    bind();
    glBindBuffer(GL_ARRAY_BUFFER, bufferID);
    glBufferSubData( GL_ARRAY_BUFFER, Vertex::offsetColor(), sizeof(glm::vec4), &color);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    unbind();
  }
